...
public class Main {
    ...
    public static void searchForBook() {
        ...
        ArrayList<Integer> indexesForContainingValues = new ArrayList<Integer>();
        boolean detailToSearchValid = false;
        System.out.println("Enter the detail you would like to use search - title (T); ISBN (I); author (A); genre (G)");
        String detailToUseToSearch = userInput().toUpperCase();
        if (!detailToUseToSearch.equals("T") && !detailToUseToSearch.equals("I") && !detailToUseToSearch.equals("A") && !detailToUseToSearch.equals("G")) {
            detailToSearchValid = true;
        }
        if (!detailToSearchValid) {
            System.out.println("Enter the value you would like to search:");
            String valueToSearch = userInput();
            System.out.println("Here are some items that directly match your search term:");
            for (int i = 0; i < readableBookDetails.length; i++) {
                if (readableBookDetails[i].equalsIgnoreCase(valueToSearch)) {
                    if (detailToUseToSearch.equals("T")) {
                    ...
                    else if (detailToUseToSearch.equals("I")) {
                        System.out.println("Title: " + readableBookDetails[i - 1] + "  ISBN: " + readableBookDetails[i] + "  Author: " + readableBookDetails[i + 1] + "  Genre: " + readableBookDetails[i + 2]);
                    }
                    else if (detailToUseToSearch.equals("A")) {
                        System.out.println("Title: " + readableBookDetails[i - 2] + "  ISBN: " + readableBookDetails[i - 1] + "  Author: " + readableBookDetails[i] + "  Genre: " + readableBookDetails[i + 1]);
                    }
                    else {
                        System.out.println("Title: " + readableBookDetails[i - 3] + "  ISBN: " + readableBookDetails[i - 2] + "  Author: " + readableBookDetails[i - 1] + "  Genre: " + readableBookDetails[i]);
                    }
                }
                else if (!detailToUseToSearch.equals("I")) {
                    if (readableBookDetails[i].toUpperCase().contains(valueToSearch)) {
                        if (detailToUseToSearch.equals("T")) {
                            indexesForContainingValues.add(i);
                        }
                        else if (detailToUseToSearch.equals("A")) {
                            indexesForContainingValues.add(i - 2);
                        }
                        else {
                            indexesForContainingValues.add(i - 3);
                        }
                    }
                }
            }
            if (indexesForContainingValues.size() > 0) {
                System.out.println("Here are some books that contain your search query:");
                for (int i : indexesForContainingValues) {
                    System.out.println("Title: " + readableBookDetails[i] + "  ISBN: " + readableBookDetails[i + 1] + "  Author: " + readableBookDetails[i + 2] + "  Genre: " + readableBookDetails[i + 3]);
                }
            }
        }
